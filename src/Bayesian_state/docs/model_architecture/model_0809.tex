...
\texttt{DynamicAdaptiveControlHypothesisTransitionModule} 的科学含义、数学定义和实施顺序作统一说明。它也不是一份结果报告：下面标为“待检验”的内容均是前瞻性计划，当前可执行配置只是实现起点，不构成科学假设已经得到支持的证据。
...
\begin{enumerate}
    ...
    \texttt{FixedWorkspaceHypothesisTransitionModule}；
...
当前 \texttt{DynamicAdaptiveControlHypothesisTransitionModule} 要求至少一个
$m$ 或 $g$ 的动态系数非零；全零控制器应绑定
\texttt{FixedWorkspaceHypothesisTransitionModule}。这避免把“参数恰好为零的动态类”误标为动态认知策略，也使静态父模型的随机过程成为清楚的嵌套端点。
...
和
\path{configs/hyper_cd_cfg/model0809_cond1_dynamic_continuous_selected8.yaml}：前者明确使用全部 trial 和 \texttt{choice\_nll}，后者为 8 名具有不同学习轨迹的 condition 1 被试（103、104、105、108、111、120、124、132）分别搜索 $M_s\in\{3,5,7\}$，并把选中的容量随其他超参数一起写入生成配置的 \texttt{subject\_overrides}。这 8 人覆盖单 block 高表现、快速突跃、平滑学习、初期回落后跃升、晚期学习、跨 session 非单调变化以及缓慢学习等开发情形；它们用于扩大工程试跑的轨迹覆盖，不构成随机抽样或确认性样本。历史 0806 sequential-holdout 配置保持不变。

\paragraph{正式模块对应。}
两步 transition 生命周期位于
\path{src/Bayesian_state/model/modules/hypothesis_transition/contracts.py}；动态控制与有限工作空间实现位于
\path{src/Bayesian_state/model/modules/hypothesis_transition/dynamic_adaptive_control.py}
和其内部 workspace policy；双记忆位于
\path{src/Bayesian_state/model/modules/memory.py}；choice/RT/oral readout 位于
\path{src/Bayesian_state/model/readout.py}；粒子积分位于
...
\path{src/Bayesian_state/simulation/execution.py}，自主行为生成在
\path{src/Bayesian_state/simulation/autonomous.py}，独立重复与汇总在
\path{src/Bayesian_state/simulation/runner.py}。choice Brier、NLL 和轨迹统计由
\path{src/Bayesian_state/metrics/} 提供；
\path{src/Bayesian_state/evaluation/} 只消费已产生的结果并执行通用或 transition 专属后处理。
