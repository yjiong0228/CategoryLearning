...
\caption{生成式有限规则搜索模型的逐试次因果结构。作答前，每个粒子依次生成内部知觉、搜索状态、有限工作空间和作答前规则信念，再由动态规则精度和被试层执行结构产生选择概率。PF 对多条不可见路径进行加权平均，得到边际预测。观察实际选择后，PF 先更新粒子权重；随后，选择与反馈共同更新规则证据、有限记忆、所有活跃规则的动态规则精度以及下一试次的搜索状态。当前反馈不能反向改变本试次已经输出的预测，虚线表示跨试次递归。}
