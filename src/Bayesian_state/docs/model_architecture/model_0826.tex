...
下文先以 condition 1 的二分类、二值反馈说明基础模型。Condition 2 已有四分类二值反馈的程序扩展，其规则数和类别概率维度应使用对应配置。Condition 3 除四分类外，还包括部分正确反馈和事先告知的层级知识；第 \ref{sec:condition3-adaptation} 节说明对应的适配机制。层级配对状态及更新已在共享核心实现，并通过定向测试和短序列验证；正式拟合、完整恢复及消融验证尚未完成。
