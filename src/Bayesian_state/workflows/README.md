...
`probe_model_0826_frozen_boundaries`复用共享PF评分和数值诊断，保留S122/S222/S315各8个历史入围点，
另加14/10/7个单独与联合外侧探针，以一档R256×B32运行，最多1760次完整PF。
它另以历史概率对S222/S315各做一次60000次重采样敏感性检查，不新增PF、不改写旧结论。
每个内侧候选都与全部外侧比较，alpha在8个预定比较之间分配；只有外侧相对所有原候选均有实质收益，
才标记为支持考虑扩范围。有限点未见收益不能证明整个范围已校准。预算到顶保留不确定，不自动补算。
协议和命令见[有限范围检查](../optimization/FROZEN_BOUNDARY_CHECK.md)。
