...
当前默认是 **v2 有界工作流程**：六人完整试点、冻结规则后三人核查及后续定向诊断均已结束。
九份完整拟合可按原状态复用，原整库独立评分通过4/9、严格整体停止0/9；剩余87人已列清单但尚未启动。
现阶段固定共同范围与预算，允许到顶保留未解决；不是宣称全体参数已验收。详见[全体第一轮工作方案](COHORT_FITTING.md)。
这也不能证明比 v1 搜索更充分或给出配对提速倍数。旧配置保存在
...
每人最多9252次完整PF，到顶仍有问题则分别保留搜索、数值或范围标记。
三人完整拟合及[范围检查](FROZEN_BOUNDARY_CHECK.md)已结束；原始结果未因追加检查改写。
...
S118仍未形成搜索平台，六人近优候选均有人为边界。后续定向检查与三人核查已完成，
不再以全部个案转为通过作为开展全体第一轮的前提；原标记必须保留，补充检查不能替代原整库审查。
...
当前已有两批共9名兼容完整结果，不能在原目录追加其余87名再 resume，因为被试列表也属于冻结指纹。
新任务另建目录，按[兼容性核查与启动清单](../../../results/model_0826/cohort_preparation_20260920/README.md)合并引用旧结果，
不将开发诊断中的新提名或外侧点替换成统一流程的原代表。
