...
| `particle_filter_evaluation.py` | particle-filter 专用评价：条件行为预测区间、ESS 与粒子边际 active-state 诊断 |
| `particle_filter_choice_transmission_audit.py` | PF choice-transmission 审计：相同粒子状态下的 hypothesis/readout 反事实 |
| `particle_filter_residual_diagnostics.py` | PF 顺序残差诊断：检验真实选择中是否仍有可预测的持续性、局部阶段和因果 residual-state 增益 |
| `transition_evaluation.py` | 仅在相应日志存在时使用的 dynamic-discrete、dynamic-continuous 与 active-set 诊断 |
...
### Strategy contribution audit

只有显式传入 `--strategy-audit-config` 时，统一 evaluation CLI 才会运行策略贡献审计，并把
结果写入 `model_evaluation/strategy_audit/`。审计用相同 PF seeds 比较三种条件：原 fitted
...
输出写入 `model_evaluation/choice_transmission_audit/`，包含逐 trial、event-aligned 和 subject
...
### Sequential residual diagnostics
