...
class StaticHypothesisStrategySpace:
...
STATIC_STRATEGY_SPACE = StaticHypothesisStrategySpace()


def _normalize_prior_assignment(spec: Mapping[str, Any]) -> dict[str, Any]:
    method = str(spec.get("method", ""))
    if method not in STATIC_STRATEGY_SPACE.prior_assignments:
        raise ValueError(
            "prior_assignment.method must be one of "
            f"{STATIC_STRATEGY_SPACE.prior_assignments}, got {method!r}."
        )
...
def _strategy_chain_kwargs(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if method != "strategy_chain":
        raise ValueError(
            "StaticHypothesisTransitionModule requires "
            "selection_strategy.method='strategy_chain'. Use "
            "StaticWorkspaceHypothesisTransitionModule for "
...
class StaticHypothesisTransitionModule(
    TwoStepHypothesisTransitionMixin,
    ...
    def __init__(self, engine, **kwargs):
        ...
        if "state_controller" in resolved:
            raise ValueError(
                "StaticHypothesisTransitionModule does not accept a state "
                "controller. Use DynamicDiscreteHypothesisTransitionModule."
            )
...
class StaticWorkspaceHypothesisTransitionModule(
    BoundedWorkspaceTransitionMixin,
    BoundedWorkspacePolicyRuntime,
):
    ...
    def __init__(self, engine, **kwargs):
        ...
        if selection_spec is not None:
            ...
            if method != "bounded_workspace":
                raise ValueError(
                    "StaticWorkspaceHypothesisTransitionModule requires "
                    "selection_strategy.method='bounded_workspace'."
                )
        ...
        if self.dynamic_rate or self.dynamic_range:
            raise ValueError(
                "StaticWorkspaceHypothesisTransitionModule requires fixed "
                "m and g. Use DynamicContinuousHypothesisTransitionModule when "
                "m_t or g_t changes across trials."
            )
        self._pending_transition: dict[str, Any] | None = None


class StaticFeedbackSwapHypothesisTransitionModule(
    TwoStepHypothesisTransitionMixin,
    BaseModule,
):
    ...
    def __init__(self, engine, **kwargs):
        ...
        if selection_spec is not None:
            ...
            if str(selection_spec.get("method", "")) != "feedback_swap_one":
                raise ValueError(
                    "StaticFeedbackSwapHypothesisTransitionModule requires "
...
FeedbackSwapHypothesisModule = StaticFeedbackSwapHypothesisTransitionModule
