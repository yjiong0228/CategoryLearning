...
class BaseDecision(BaseModule):
    """Compatibility interface for legacy explicit choice sampling."""
...
class Decision(BaseDecision):
    """Sample or maximize an already-computed choice distribution."""

    def __init__(self, engine, **kwargs):
        super().__init__(engine, **kwargs)
        self.method = kwargs.get("method", "sample")
        module_seed = kwargs.get("module_seed")
        self.rng = None if module_seed is None else np.random.default_rng(module_seed)

    def decision(self, probability: np.ndarray, **kwargs) -> int:
        method = kwargs.get("method", self.method)
        normalized = normalize_probability_vector(probability, strict=True)
        if method == "top":
            return int(np.argmax(normalized))
        if self.rng is None:
            return int(np.random.choice(normalized.size, p=normalized))
        return int(self.rng.choice(normalized.size, p=normalized))

    def process(self) -> None:
        # Kept as a no-op for legacy agendas; new code calls decision() directly.
        return None
