...
def _choice_layer_audit(
    ...
    *,
    ...
    fitted = _normalize(
        (1.0 - float(lapse)) * persistent_execution_no_lapse
        + float(lapse) / float(n_categories)
    )
...
def run_state_model_particle_filter(
    *,
    ...
    execution_flags = [
        bool(
            getattr(
                model.engine.modules["hypo_transitions_mod"],
    ...
    for trial_index in range(n_trials):
        ...
        for particle_index, model in enumerate(models):
            ...
            transition = engine.modules["hypo_transitions_mod"]
            if bool(
                getattr(transition, "uses_outcome_feedback_controller", False)
            ) and hasattr(transition, "record_outcome"):
                transition.record_outcome(engine.observation)
            if hasattr(engine, "clear_module_logs"):
                engine.clear_module_logs()
