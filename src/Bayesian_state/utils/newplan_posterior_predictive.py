...
from .newplan_particle_filter import (
    _choice_probability,
    _future_seed,
    _normalize,
    _particle_config,
...
def run_conditioned_condition1_rollouts(
    *,
        ...
        for particle_index in range(n_particles):
            trait_rng = np.random.default_rng(
                _future_seed(
                    ...
                    -1,
                    particle_index,
                    "newplan_ppc_dynamic_rho_traits",
                )
            )
    ...
    for trial_index in range(split_index):
        if hazard_value is not None:
            for particle_index in np.flatnonzero(~acquired):
                acquisition_seed = _future_seed(
                    ...
                    trial_index,
                    int(particle_index),
                    "newplan_ppc_acquisition_change_point",
                )
            ...
            for particle_index in range(n_particles):
                innovation_rng = np.random.default_rng(
                    _future_seed(
                        ...
                        trial_index,
                        particle_index,
                        "newplan_ppc_dynamic_rho_prefix_innovation",
                    )
                )
        ...
        for particle_index, model in enumerate(models):
            ...
            update_seed = _future_seed(
                ...
                trial_index,
                particle_index,
                "newplan_ppc_learning_update_gate",
            )
            update_occurs = bool(
        ...
        if post_ess[trial_index] < threshold_fraction * n_particles:
            seed = _future_seed(
                ...
                trial_index,
                0,
                "newplan_ppc_systematic_resampling",
            )
    ...
    ancestor_rng = np.random.default_rng(
        stable_seed(
            {
                "seed_role": "newplan_ppc_boundary_ancestor",
    ...
    for rollout_index, ancestor in enumerate(ancestor_indices):
        ...
        acquisition_rng = np.random.default_rng(
            stable_seed(
                {
                    "seed_role": "newplan_ppc_rollout_acquisition",
                    "trajectory_seed": int(trajectory_seed),
                }
            )
        )
        ...
        rho_rng = np.random.default_rng(
            stable_seed(
                {
                    "seed_role": "newplan_ppc_rollout_dynamic_rho",
                    "trajectory_seed": int(trajectory_seed),
                }
            )
        )
        for local_index, trial_index in enumerate(
            ...
            update_seed = _future_seed(
                ...
                trial_index,
                rollout_index,
                "newplan_ppc_rollout_learning_update_gate",
            )
            update_occurs = bool(
                np.random.default_rng(update_seed).random()
                < update_probability
