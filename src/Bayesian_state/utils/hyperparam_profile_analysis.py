    ...
    for condition, root in condition_roots.items():
        ...
        for sid_text, record in sorted(outer.items(), key=lambda item: int(item[0])):
            ...
            row = {
                ...
                "outer_hyper_candidate_seed": record.get("hyper_candidate_seed", record.get("random_seed")),
                "outer_random_seed": record.get("random_seed", record.get("hyper_candidate_seed")),
