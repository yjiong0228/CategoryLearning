"""Autonomous trajectory generation for the minimal new-plan model."""
...
from .optimizer_common import inject_module_seed_from_trajectory, stable_seed
...
def generate_condition1_trajectory(
    *,
    ...
    if not hasattr(transition, "record_outcome_feedback"):
        raise ValueError(
            "Autonomous new-plan generation requires FeedbackSwapHypothesisModule."
        )
    ...
    choice_seed = stable_seed(
        {
            "seed_role": "newplan_autonomous_choice",
            "trajectory_seed": int(trajectory_seed),
        }
    )
    perception_seed = stable_seed(
        {
            "seed_role": "newplan_autonomous_perception",
            "trajectory_seed": int(trajectory_seed),
        }
    )
