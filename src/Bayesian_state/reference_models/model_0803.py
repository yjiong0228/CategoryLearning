...
def build_partition(condition: int) -> ContinuousPartition:
    """Build the fixed-label geometric rule space used by model_0803."""

    return ContinuousPartition(
        n_dims=4,
        n_cats=n_categories(condition),
    )


def partition_prior(partition: ContinuousPartition, mode: str = "uniform_rule") -> np.ndarray:
    ...
    families = np.asarray(
        [hypothesis.family for hypothesis in partition.hypothesis_space],
        dtype=object,
    )
