...
`configs/` 和 `optimization/` 启动；可复用的认知机制应迁入 `model/modules/`。
...
| `model_0804/core.py` | hard finite workspace、FA0/FA1/FA2/FA2R、粒子/精确/alive/resample-move 推断、RT likelihood |
| `model_0804/recovery.py` | 0804 autonomous choice/feedback recovery simulation |
| `model_0804/forgetting.py` | filtered anchor state 与 coupled forgetting history 分析 |
| `model_0804/pgas.py` | innovation path replay、exact smoothing 和 PGAS |
| `model_0806.py` | surprise/uncertainty-driven FA3-M autonomous simulation 与 RT emission |

`model_0804/core.py` 是该系列的数值基础；0806 复用其中的 finite-workspace state、transition 和
feedback update。
...
```text
model/modules/hypothesis_transition/dynamic_adaptive_control.py
inference/backends/particle_filter.py
...
因此，“主框架结果”和“reference-model numerical result”不能仅凭同名参数假定逐 trial 完全相同；
