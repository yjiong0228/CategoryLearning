...
class HypothesisSelectionPolicy(PriorAssignmentPolicyMixin, BaseModule):
    """
    ...
    def __init__(self, engine, **kwargs):
        ...
        if "init_pool" in kwargs:
            raise ValueError(
                "init_pool has been removed with label-permuted hypotheses; "
                "initial hypotheses are sampled from the full geometric-rule space."
            )
    ...
    def _validate_ksimilar_partition(self) -> None:
        ...
        if missing:
            raise ValueError(
                ...
                "multi-prototype category access. "
    ...
    def _cluster_strategy_ksimilar_centers(self,
        ...
        trial_data = ([stimulus], [1], [1.0])
        ref_probs = np.asarray(
            [
                self.engine.partition.get_category_probabilities(
                    int(hypothesis),
                    trial_data,
                    beta=float(ref_hypos_beta[index]),
                    distance_mode="prototype",
                )[:, 0]
                for index, hypothesis in enumerate(ref_hypos_index)
            ],
            dtype=float,
        )
        ...
        # For a disconnected category, use the component prototype closest to
        # the current stimulus. This is the prototype actually supporting the
        # sampled category at this trial.
        ref_hypos_center = []
        for hypothesis, choice in zip(ref_hypos_index, ref_choices):
            prototypes = self.engine.partition.get_category_prototypes(
                int(hypothesis),
                int(choice),
            )
            nearest = int(np.argmin(np.linalg.norm(prototypes - stimulus, axis=1)))
            ref_hypos_center.append(prototypes[nearest])
        ref_hypos_center = np.asarray(ref_hypos_center, dtype=float)
    ...
    def _calc_cached_dist(self):
        ...
        if not hasattr(self.engine.partition, "get_category_prototypes"):
             return

        self.cached_dist = {}
        # Distances are populated lazily by center_dist for the selected
        # component prototypes rather than for a fixed one-center array.
        pass
    ...
    def _init_mask(self) -> None:
        ...
        if self.init_hypotheses is not None:
            ...
            if forced.size >= self.init_num:
                ...
                pool = self._exclude(self.full_indices, forced)
...
__all__ = ["HypothesisSelectionPolicy"]
