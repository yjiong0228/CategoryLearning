...
class M_Base:
    ...
    def get_centers(self, k: int, condition: int) -> List[np.ndarray]:
        ...
        centers = self.all_centers[
            '2_cats'] if condition == 1 else self.all_centers['4_cats']
        if 1 <= k <= len(centers):
    ...
    def likelihood(self, params: ModelParams, data,
        ...
        distances = np.linalg.norm(x[:, np.newaxis, :] - np.array(centers),
                                   axis=2)  # Shape: [nTrials, nCategoris]

        probs = np.exp(-beta * distances)  # Shape: [nTrials, nCategoris]
        probs /= np.sum(probs, axis=1,
                        keepdims=True)  # Normalize probabilities
        p_c = probs[np.arange(len(c)),
                    c - 1]  # Probability of chosen category (shape: [nTrials])
    ...
    def posterior(self, params: ModelParams, data, condition: int) -> float:
        ...
        log_likelihood = np.sum(
            np.log(self.likelihood(params, data, condition)))
    ...
    def fit(self, data) -> Tuple[ModelParams, float, float, Dict]:
        ...
        for k in range(1, max_k + 1):
            result = minimize(lambda beta: self.posterior(
                ModelParams(k, beta[0]), data, condition),
            ...
            log_likelihood = np.sum(
                np.log(
                    self.likelihood(ModelParams(k, beta_opt), data,
                                    condition)))
        ...
        k_posteriors = {
            k: np.exp(log_p - max_log_posterior)
            for k, log_p in k_posteriors.items()
        }
