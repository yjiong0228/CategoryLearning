...
def test_strategy_audit_static_counterfactuals_are_mean_matched_and_fixed() -> None:
    ...
    assert transition["class"].endswith("FixedWorkspaceHypothesisTransitionModule")
    ...
    payload = {
        ...
        "representative_run": {
            ...
            "state_log": {
                ...
                "predictive_replacement_fraction": [0.0, 0.10, 0.20, 0.40],
...
def test_dynamic_continuous_particle_plots_and_basic_dispatch(tmp_path: Path, monkeypatch) -> None:
    ...
    plot_methods = (
        ...
        (evaluator.plot_particle_filter_active_set_counts, "active_counts.png"),
    )
    ...
    assert {
        ...
        "hypothesis_active_set_counts.png",
    }.issubset(called)
...
def test_particle_filter_active_set_counts_use_pre_choice_replacement_fraction():
    ...
    info = {
        ...
        "predictive_replacement_fraction": [0.0, 0.25, 0.50, 0.75],
        "replacement_fraction": [0.0, 0.0, 0.0, 0.0],
    }

    counts = evaluator._particle_filter_active_set_count_data(info)

    assert counts["capacity"] == 4
    assert counts["source_semantics"] == "pre_choice_particle_marginal"
    np.testing.assert_allclose(counts["raw"]["newcomer"], [0.0, 1.0, 2.0, 3.0])
    np.testing.assert_allclose(counts["raw"]["retained"], [4.0, 3.0, 2.0, 1.0])
    np.testing.assert_allclose(counts["raw"]["total"], 4.0)
    np.testing.assert_allclose(counts["smooth"]["newcomer"], [0.0, 0.5, 1.5, 2.5])
...
def test_particle_filter_accuracy_band_uses_behavioral_draws(
    tmp_path: Path,
    monkeypatch,
) -> None:
    _write_particle_subject(tmp_path)
